...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .balance-amount {
            font-size: 22px; font-weight: 700; color: #fff; letter-spacing: -0.3px; line-height: 1;
        ...
        .withdraw-header-btn { background: #2a2a3a; color: #c4b5fd; border: 1px solid #3a3a55; padding: 7px 12px; border-radius: 12px; font-size: 11px; font-weight: 500; cursor: pointer; white-space: nowrap; transition: 0.2s; }
        ...
        .tap-container { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; gap: 14px; padding: 16px 0 10px; }
        ...
        .coin-outer-glow {
            ...
            top: -38px; left: -38px; pointer-events: none; z-index: 0;
            transition: all 0.5s ease;
        }
        .coin-outer-glow.active {
            box-shadow: 0 0 60px 15px rgba(184,169,255,0.12), 0 0 120px 30px rgba(184,169,255,0.06);
        }
        .coin-outer-glow.limit {
            box-shadow: 0 0 60px 15px rgba(252,165,165,0.12), 0 0 120px 30px rgba(252,165,165,0.06);
        }
        .coin-glow {
            ...
            top: -14px; left: -14px; pointer-events: none; z-index: 0;
        ...
        .coin {
            ...
            box-shadow: 0 0 0 6px #1a1a24, 0 20px 60px rgba(0,0,0,0.7);
            display: flex; align-items: center; justify-content: center;
            cursor: pointer; transition: transform 0.06s ease; position: relative; z-index: 1;
        }
        .coin:active { transform: scale(0.88); }
        .coin.blocked { border-color: #fca5a5; box-shadow: 0 0 0 6px #1a1a24, 0 20px 60px rgba(0,0,0,0.7), 0 0 35px rgba(252,165,165,0.35); }
        .coin-text { font-size: 22px; font-weight: 700; color: #c4b5fd; letter-spacing: 3px; }
        .block-timer { font-size: 12px; color: #fca5a5; text-align: center; min-height: 18px; }
        .tap-plus { position: fixed; font-size: 24px; font-weight: 700; color: #c4b5fd; pointer-events: none; z-index: 200; animation: floatUp 0.8s ease-out forwards; }
        @keyframes floatUp { 0% { opacity: 1; transform: translateY(0) scale(0.7); } 30% { opacity: 1; transform: translateY(-45px) scale(1.3); } 100% { opacity: 0; transform: translateY(-120px) scale(0.8); } }
        ...
        .progress-bar { width: 100%; max-width: 280px; height: 6px; background: #1e1e2a; border-radius: 6px; overflow: hidden; position: relative; }
        .progress-fill { height: 100%; background: linear-gradient(90deg, #7c6fd4, #b8a9ff); border-radius: 6px; transition: width 0.4s ease; position: relative; }
        ...
        .support-inline {
            ...
            width: 100%; max-width: 420px; z-index: 99;
        }
        .support-inline a {
...
<body>
    ...
    <div class="modal-overlay" id="withdrawModal">
        <div class="modal">
            ...
            <h3>Вывод средств</h3>
            <input class="withdraw-input" type="number" id="withdrawAmount" placeholder="Сумма (мин 1500)" min="1500" inputmode="decimal" oninput="updateWithdrawInfo()">
            ...
            <div id="methodDetails" class="hidden"></div>
    ...
    <script>
        const STORAGE_KEY='semaka_v13', ADMIN_PASSWORD='admin123', MIN_TAP_INTERVAL=100, CAPTCHA_EVERY=40, BLOCK_DURATION=3000, MAX_FAST_TAPS=5;
        ...
        function getGlobalStore(){let s=localStorage.getItem('semaka_global_v8');if(!s){s={users:{},withdrawRequests:[],adminLogs:[],totalTaps:{},tasks:[{id:'task_default',name:'Подписаться на канал',reward:100,link:'https://t.me/example',description:'Перейди и подпишись'}],broadcasts:[]};localStorage.setItem('semaka_global_v8',JSON.stringify(s));}else s=JSON.parse(s);return s;}
        function saveGlobalStore(s){localStorage.setItem('semaka_global_v8',JSON.stringify(s));}
        ...
        function updateGlowState() {
            const glow = document.getElementById('coinOuterGlow');
            glow.classList.remove('active', 'limit');
            if (data.tapsToday >= 200) { glow.classList.add('limit'); }
            else { glow.classList.add('active'); }
        }

        function spawnParticles(x,y){const colors=['#c4b5fd','#b8a9ff','#9d8fff','#f0b90b','#fff'];for(let i=0;i<8;i++){const p=document.createElement('div');p.className='coin-particle';const angle=Math.random()*Math.PI*2;const dist=25+Math.random()*50;p.style.setProperty('--dx',Math.cos(angle)*dist+'px');p.style.setProperty('--dy',Math.sin(angle)*dist+'px');p.style.left=x+'px';p.style.top=y+'px';p.style.background=colors[Math.floor(Math.random()*colors.length)];p.style.width=(5+Math.random()*8)+'px';p.style.height=p.style.width;document.body.appendChild(p);setTimeout(()=>p.remove(),600);}}
        ...
        async function handleTap(event){
            ...
            lastTapTime=now;
            tapsSinceCaptcha++;if(tapsSinceCaptcha>=CAPTCHA_EVERY){tapsSinceCaptcha=0;if(!await showCaptcha()){blockUser('Провал капчи');addLog('captcha_fail','Капча: '+data.userId);return;}}
            data.tapsToday++;
            ...
            const plus=document.createElement('div');plus.className='tap-plus';plus.textContent='+'+reward.toFixed(1);plus.style.left=cx+'px';plus.style.top=(cy-26)+'px';document.body.appendChild(plus);setTimeout(()=>plus.remove(),800);
            animateBalance();
            updateUI();
        }
        ...
        function renderLeaderboard(){const c=document.getElementById('leaderboard');if(!c)return;let users=[];for(let uid in globalStore.users)users.push({id:uid,name:(globalStore.users[uid].username||uid).substring(0,16),taps:globalStore.totalTaps[uid]||0,refs:(globalStore.users[uid].referrals||[]).length,isYou:uid===data.userId});
            const podium=(i)=>{
                if(i===0)return'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f0b90b" stroke-width="2"><path d="M4 20h16"/><path d="M8 20V12"/><path d="M16 20V12"/><path d="M12 20V4"/></svg>';
                if(i===1)return'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#aaa" stroke-width="2"><path d="M4 20h16"/><path d="M8 20V12"/><path d="M16 20V12"/><path d="M12 20V8"/></svg>';
                if(i===2)return'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#cd7f32" stroke-width="2"><path d="M4 20h16"/><path d="M8 20V14"/><path d="M16 20V14"/><path d="M12 20V10"/></svg>';
                return`<span style="font-size:13px;color:#666;">${i+1}</span>`;
            };
        ...
        function onMethodChange(){
            ...
            const details=document.getElementById('methodDetails'),fee=document.getElementById('feeDetails');
            details.classList.remove('hidden');fee.classList.remove('hidden');
            if(method==='sbp'){details.innerHTML='<select class="withdraw-select" id="bankSelect"><option value="">Выберите банк</option><option>Сбербанк</option><option>Т-Банк</option><option>ВТБ</option><option>Альфа-Банк</option><option>Райффайзен</option></select><input class="withdraw-input" type="text" id="dynamicWalletInput" placeholder="Введите номер телефона">';}
            else if(method==='card'){details.innerHTML='<select class="withdraw-select" id="cardType"><option value="">Тип карты</option><option>VISA</option><option>Mastercard</option><option>Мир</option></select><input class="withdraw-input" type="text" id="dynamicWalletInput" placeholder="Введите номер карты">';}
            else if(method==='ton'){details.innerHTML='<input class="withdraw-input" type="text" id="dynamicWalletInput" placeholder="Введите адрес кошелька в сети TON">';}
            else if(method==='usdt'){details.innerHTML='<input class="withdraw-input" type="text" id="dynamicWalletInput" placeholder="Введите адрес кошелька TRC-20">';}
            else if(method==='cryptobot'){details.innerHTML='<select class="withdraw-select" id="cryptoCurrency"><option value="">В какой монете желаете получить чек?</option><option>USDT</option><option>TON</option></select><input class="withdraw-input" type="text" id="dynamicWalletInput" placeholder="Введите ID Crypto Bot">';}
            else{details.classList.add('hidden');fee.classList.add('hidden');}
            updateWithdrawInfo();
        }
        function updateWithdrawInfo(){
            const amt=parseFloat(document.getElementById('withdrawAmount').value)||0,method=document.getElementById('withdrawMethod').value;
            const feeEl=document.getElementById('feeText'),totalEl=document.getElementById('totalAmount'),feeDiv=document.getElementById('feeDetails');
            if(!method||amt<1500){feeDiv.classList.add('hidden');return;}feeDiv.classList.remove('hidden');
            let fee=0,feeText='';
            if(method==='sbp'||method==='card'||method==='cryptobot'){fee=0;feeText='Без комиссии';}
            else if(method==='ton'){fee=5;feeText='Комиссия сети: ~5 монет';}
            else if(method==='usdt'){fee=Math.round(amt*0.05+350);feeText='Комиссия: 350₽ + 5% (~'+fee+' монет)';}
            feeEl.innerHTML=feeText;totalEl.textContent=Math.max(0,amt-fee).toFixed(0);
        }

        function openWithdrawModal(){if(isProcessingAction)return;document.getElementById('withdrawModal').classList.add('active');document.getElementById('withdrawMsg').textContent='';document.getElementById('withdrawAmount').value='';document.getElementById('withdrawMethod').value='';document.getElementById('methodDetails').classList.add('hidden');document.getElementById('feeDetails').classList.add('hidden');renderWithdrawHistory();}
        function closeWithdrawModal(){document.getElementById('withdrawModal').classList.remove('active');}
        function renderWithdrawHistory(){const h=data.withdrawHistory||[];const c=document.getElementById('withdrawHistory');if(h.length===0){c.innerHTML='<p style="color:var(--sub);font-size:11px;text-align:center;">Нет истории</p>';return;}c.innerHTML=h.slice().reverse().map(r=>`<div class="history-item"><div><div style="font-size:11px;">${r.amount} монет — ${r.method||'Способ'}</div><div style="font-size:9px;color:var(--sub);">${r.date}</div></div><span class="history-status ${r.status}">${r.status==='processing'?'В обработке':r.status==='paid'?'Выплачено':r.status==='rejected'?'Отклонено':r.status}</span></div>`).join('');}
        function requestWithdraw(){
            ...
            const walletInput=document.getElementById('dynamicWalletInput'),wallet=walletInput?walletInput.value.trim():'';
            const bankSelect=document.getElementById('bankSelect'),bank=bankSelect?bankSelect.value:'';
            const cardType=document.getElementById('cardType'),card=cardType?cardType.value:'';
            const cryptoCurrency=document.getElementById('cryptoCurrency'),crypto=cryptoCurrency?cryptoCurrency.value:'';
            ...
            if(!method){msg.textContent='Выбери способ';btn.disabled=false;isProcessingAction=false;return;}
            ...
            let desc='';if(method==='sbp')desc=`СБП (${bank||'Банк'}) → ${wallet||'телефон'}`;else if(method==='card')desc=`Карта ${card||'VISA'} → ${wallet}`;else if(method==='cryptobot')desc=`Crypto Bot (${crypto||'Валюта'}) → ${wallet}`;else desc=`${method.toUpperCase()} → ${wallet}`;
            ...
            document.getElementById('methodDetails').classList.add('hidden');document.getElementById('feeDetails').classList.add('hidden');
            renderWithdrawHistory();animateBalance();updateUI();
        ...
        function processWithdraw(id,status){const req=globalStore.withdrawRequests.find(r=>r.id===id);if(req){req.status=status;req.adminNote=status==='paid'?'Выплачено':'Отклонено';}const uid=req?.userId;for(let k in localStorage){if(k.startsWith('semaka_v13')){try{const ud=JSON.parse(localStorage.getItem(k));if(ud.userId===uid&&ud.withdrawHistory){const hi=ud.withdrawHistory.find(h=>h.id===id);if(hi){hi.status=status;hi.adminNote=req.adminNote;localStorage.setItem(k,JSON.stringify(ud));}}}catch(e){}}}saveGlobalStore(globalStore);addLog('withdraw_'+status,`Заявка ${id}`);renderAdminPanel();}
